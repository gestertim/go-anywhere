...
export function EditItineraryForm({ item }: { item: ItineraryItem }) {
  ...
  return <form action={action}>
    ...
    <fieldset><legend>地點資訊</legend><label>地點名稱<input ref={placeNameRef} name="placeName" defaultValue={item.place?.name ?? ""} /></label><label>地址<input ref={addressRef} name="address" defaultValue={item.place?.address ?? ""} /></label><label>緯度<input ref={latitudeRef} name="latitude" type="number" step="any" defaultValue={item.place?.latitude ?? ""} /></label><label>經度<input ref={longitudeRef} name="longitude" type="number" step="any" defaultValue={item.place?.longitude ?? ""} /></label></fieldset>
