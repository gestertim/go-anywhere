...
export default async function NewItineraryPage({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ type?: string }> }) {
  ...
  return <main><Link href={`/trips/${tripId}`}>返回旅程</Link><h1>新增行程</h1>{selected ? <ItineraryForm tripId={tripId} type={selected} /> : <><p>先選擇行程類型</p><ItineraryTypePicker tripId={tripId} /></>}</main>;
}
