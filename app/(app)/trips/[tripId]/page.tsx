...
export default async function TripWorkspacePage({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ view?: string; date?: string }> }) {
  ...
  return (
    <main>
      ...
      <nav aria-label="旅程檢視"><Link aria-current={view === "timeline" ? "page" : undefined} href={withView("timeline")}>時間軸</Link><Link aria-current={view === "map" ? "page" : undefined} href={withView("map")}>地圖</Link><Link aria-current={view === "bookings" ? "page" : undefined} href={withView("bookings")}>預訂</Link><Link aria-current={view === "notes" ? "page" : undefined} href={withView("notes")}>筆記</Link></nav>
